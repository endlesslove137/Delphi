...
begin
  ...
  try
    ...
    Ident := '副本列表';
    FubenListXML := FConfigINI.ReadString(Section,Ident,'http://192.168.0.8/data/lang/zh-cn/fubennewClient.cbp');
    if not FConfigINI.ValueExists(Section,Ident) then
      FConfigINI.WriteString(Section,Ident,FubenListXML);

    Ident := '活动列表';
    activityListXML := FConfigINI.ReadString(Section,Ident,'http://192.168.0.8/data/lang/zh-cn/activity.cbp');
    if not FConfigINI.ValueExists(Section,Ident) then
      FConfigINI.WriteString(Section,Ident,activityListXML);
