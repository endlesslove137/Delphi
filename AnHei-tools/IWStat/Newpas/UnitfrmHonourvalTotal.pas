...
type
  TIWfrmHonourvalTotal = class(TIWfrmWebGrid)
    IWRegion2: TIWRegion;
    IWButton3: TIWButton;
...
var
  IWfrmHonourvalTotal: TIWfrmHonourvalTotal;
...
procedure TIWfrmHonourvalTotal.IWAppFormCreate(Sender: TObject);
const
  sqlsex = 365;
  sqlMoneyRank = 'select accountname, actorname, IF(sex=0,';
  sqlMoneyRank2 = '), %s,circle,level, (honourval) from actors where (status & 2) = 2 order by  honourval desc limit %d';
var
...
begin
  ...
  TIWAdvWebGrid1.Columns[7].Title:= Langtostr(343);
  try
...
procedure TIWfrmHonourvalTotal.IWButton3Click(Sender: TObject);
var
  sFileName: string;
begin
  sFileName := 'HonourvalTotal' + DateToStr(Now) + '.csv';
...
initialization
  RegisterClass(TIWfrmHonourvalTotal);
