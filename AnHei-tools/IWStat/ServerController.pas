...
var
  ...
  ActivityList: TStringList;
  FubenList: TStringList;
...
begin
  ...
  try
    ...
    ActivityList := TStringList.Create;
    FubenList := TStringList.Create;
...
begin
  ...
  EnterCriticalSection(IWServerController.m_SessionLoadLock);
  try
    LuaField := cbp.LoadHttpCBP(objINI.ActivityListXML);
    ClearActivity;
    for I := 0 to LuaField.FieldCount-1 do
    begin
      TableField := LuaField.FieldIndex[I].AsTable;
      try
        ...
        begin
          New(PActivity1);
          PActivity1^.id := TableField.Fields['id'].AsInteger;
          PActivity1^.name := ShortString(TableField.Fields['name'].AsString);
          ActivityList.AddObject(string(PActivity1^.name),TObject(PActivity1))
        end;
      except
        continue;
      end;
...
var
  I: Integer;
begin
  for I := 0 to ActivityList.Count - 1 do
  begin
    System.Dispose(PActivity(ActivityList.Objects[I]));
  end;
  ActivityList.Clear;
end;
...
begin
  ...
  EnterCriticalSection(IWServerController.m_SessionLoadLock);
  try
    LuaField := cbp.LoadHttpCBP(objINI.fubenListXML);
    ClearFuben;
    for I := 0 to LuaField.FieldCount-1 do
    begin
      TableField := LuaField.FieldIndex[I].AsTable;
      try
        ...
        begin
          New(Pfuben1);
          Pfuben1^.id := TableField.Fields['fbid'].AsInteger;
          Pfuben1^.name := ShortString(TableField.Fields['fbname'].AsString);
          FubenList.AddObject(string(Pfuben1^.name),TObject(Pfuben1))
        end;
      except
        continue;
      end;
...
var
  I: Integer;
begin
  for I := 0 to FubenList.Count - 1 do
  begin
    System.Dispose(PFuben(FubenList.Objects[I]));
  end;
  FubenList.Clear;
end;
