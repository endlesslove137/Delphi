...
uses Windows, SysUtils, Classes, lua, UnitLangPackage, Dialogs;
...
var
  LuaLang: TLuaLanguagePackage;  //语言包全局对象
  iFileCount: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not boFound then
      begin
        sFile := extractfilepath(sScritpFileName) + sInclude;
        boFound := FileExists(  sFile );
      end;
//      if not boFound then
//      begin
...
//      end;
      ...
      if not boFound then
      begin      
        sFile := 'D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\!SC\Server\'+ sInclude;
        boFound := FileExists(  sFile );
      end;
      if boFound then
      begin
...
begin
  ...
  begin
    try
      ...
      on E: Exception do
      begin
        showmessage('TBaseLuaScript.SetScript(' + E.Message + ')');
      end;
    end;
    m_Lua := nil;
  end;
