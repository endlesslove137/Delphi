...
uses
  ...
  DBXMySQL, dateutils, DBXSybaseASA, comobj, activex,pngimage;

const
 Servertype : array[0..2] of string = ('中国', '韩国', '越南');
 ServerRootPath = 'D:\mine\百度网盘\我的文档\work\program\521g\idgp\ZhanJiangII\!SC\';
 ...
 //战千雄游戏充值金额排名前50玩家账号
 sqlorder50 ='select account,sum(rmb) as rmb,orderdate from %s.payorder where type in (1,3) and yunying = "%s"  GROUP BY account ORDER BY rmb DESC limit 0,50';
...
var
 strSql, saccount, srmb, sorderdate: string;
 list: TStringList;
 temp: Integer;
begin
  try
    ...
    begin
      ...
      begin
       ...
       list.Add(saccount + #9 + srmb + #9 + sorderdate + #9 + ServerData.ServerName);
       Next;
      end;
    end;
  finally
...
//  Serverdata.orderlist.Add(Serverdata.ServerName+'金额排名前50' + #9 + '账号' + #9 + '充值金额'+ #9 + '充值日期');
...
//  TempServer := Serverdata;
  Serverdata.SumText.Clear;
//  Serverdata.SumText.Add(Serverdata.ServerName + #9 + '月流水' + #9 + '月均在线'+ #9 + '月付费用户'+ #9 + '月ARPU'+ #9 +'月底注册用户数');
...
//  entercriticalsection(cs);
  CoInitialize(nil);
//   Serverdata.SumText.Add(formatdatetime('YYYY年MM月', date) + #9 + inttostr(SumReChange)+ #9 + inttostr(SumMonthOnline)+ #9 + inttostr(SumMonthPayUser)+ #9 + inttostr(SumMonthARPU) + #9 + inttostr(SumMonthRegister));
...
begin
  try
    ...
    ExcelApp.WorkBooks.Open(ServerExcel);
...
//  png.LoadFromFile('D:\mine\百度网盘\我的文档\work\program\521g\tcgp\trunk\AnHei\tools\Launcher\Release\config\main1.png');
  png.LoadFromFile('D:\mine\百度网盘\我的文档\work\program\521g\tcgp\trunk\AnHei\tools\Launcher\Release\config\main1.png');
...
begin
 ...
 try
   ...
   begin
    ...
    tempStr := inttostr(i) + ConnectionStr + tempStr;
...
begin
  mmo2.Text := 'D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\!SC\tools\AHXYAddtion\AHXYEditor.map' + #13#10
              +'D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\!SC\tools\AHXYAddtion\AHXYEditor.exe';
  btn3Click(nil);
  btn2click(nil);
end;
...
begin
 mmo2.Text :='D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\!SC\tools\TaskEditor\TaskEditor.exe' + #13#10
           + 'D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\!SC\tools\TaskEditor\TaskEditor.map';
 btn3Click(nil);
 btn2click(nil);
end;
...
begin
 ...
 strTemp := StringReplace(strTemp, 'D:\mine\百度网盘\我的文档\work\program\521g\idgp\AnHei\', '', [rfReplaceAll]);
 pathList.Add(strTemp);


 pathList.Add(#13#10 + '操作步骤 ');
 i := 0;
 if pos( 'ZuoCi.txt', TagetMemo.Text) > 0 then
 begin
   inc(i);
