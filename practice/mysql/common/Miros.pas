...
function GetHardNumber: pchar;
function GetCpuBuilder : Pchar;
function GetCpuIdA : integer;
function GetCPUID(EAX: DWord): TCPUIDResult;
function GetCPUID_Str(EAX: DWord): PCHar;
function SoundInstall:Boolean;   //声卡是否安装
function TickCountToStr(TickCount	:	Cardinal):Pchar;
...
procedure AddRich(IO:TRichEdit;Str:Pchar;FontClr: TColor = clBlack; GroundClr:Tcolor = clWhite;FontName:Tfontname = '宋体';FontSize:Byte = 12;Udline:Boolean = false);//向richedit加彩色背景文字;
//procedure AddRich(aRichEdit:TRichEdit; Text:Pchar; Font : TFont);
procedure AddRichA(RichEdit : TRichEdit; Text : Pchar; aFont : TFont; BackColor : TColor; UnderLine : Boolean);
...
procedure FindAllFile(Path, DefExt: string; DestResult : TStrings);
...
begin
  ...
  try
    ...
    Buffer:=GetLocalName;
    HostEnt:=GetHostByName(pchar(Buffer));
...
procedure AddRich(IO:TRichEdit;Str:Pchar;FontClr: TColor = clBlack; GroundClr:Tcolor = clWhite;FontName:Tfontname = '宋体';FontSize:Byte = 12;Udline:Boolean = false);
...
begin
 ...
 result := PChar(Format('%d-%d-%d-%d',[rEAX,rEBX,rECX,rEDX]));
end;
...
function TickCountToStr(TickCount	:	Cardinal):Pchar;
var
  tempday,temphour,tempmin,tempsec : word;
  j : Dword;
begin
  Result := 'UNDEFINED';
  tempday   :=    0;
  ...
  if tempmin > 0 then
    result := Pchar(format('%2d分%2d秒 ',[tempmin,tempsec]));

  if temphour > 0 then
    result := Pchar(format('%2d小时%2d分%2d秒 ',[temphour,tempmin,tempsec]));

  if tempday >  0 then
    result := Pchar(format('%2d天%2d小时%2d分%2d秒 ',[tempday,temphour,tempmin,tempsec]));
end;
