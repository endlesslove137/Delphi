...
function CalcFileCRC(FileName: string): LongWord;
...
begin
  Result := $FFFFFFFF;
  hFile := FileOpen(FileName, fmShareDenyWrite);
  if hFile <> INVALID_HANDLE_VALUE then
  begin
    while True do
    begin
      nSize := FileRead(hFile, Buf[0], sizeof(Buf));
      if nSize = 0 then Break;
      Result := CRC32(Result, @Buf[0], nSize);
    end;
    FileClose( hFile );
  end;
  Result := not Result;
